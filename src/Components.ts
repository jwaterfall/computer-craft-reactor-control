...
export class Page implements Component {
  ...
  public render(monitor: Monitor): void {
    ...
    monitor.drawText(
      Math.floor((width - this.title.length) / 2),
      1,
      this.title,
      colors.blue,
      colors.white
...
export class HorizontalLayout implements Component {
  ...
  public render(
    ...
    const childWidth = Math.floor(
      (width - (this.children.length - 1) * this.gap) / this.children.length
    );
    ...
    this.children.forEach((child, index) => {
      child.render(
        monitor,
        x + index * (childWidth + this.gap + extraGap),
        y,
        childWidth,
        height
      );
...
export class VerticalLayout implements Component {
  ...
  public render(
    ...
    const childHeight = Math.floor(
      (height - (this.children.length - 1) * this.gap) / this.children.length
    );
...
export class ProgressBar implements Component {
  public constructor(
    ...
    private title: string,
    private color: Color,
    private startPercentage = 0,
    private endPercentage = 1
  ) {}
  ...
  public render(
    ...
    const percentageRange = this.endPercentage - this.startPercentage;
    const normalizedPercentage =
      (percentage - this.startPercentage) / percentageRange;

    const clampedPercentage = Math.min(Math.max(normalizedPercentage, 0), 1);

    const barWidth = Math.floor(clampedPercentage * width);
    ...
    monitor.drawText(
      x + 1,
      y,
      `${this.title} ${Math.round(percentage * 100)}%`
    );
  }
}
...
export class Button implements Component {
  public constructor(
    ...
    private event?: MonitorTouchEvent,
