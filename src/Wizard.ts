...
import { findMonitorPeripherals, findReactorPeripheral, findTurbinePeripherals } from "./Peripheral";
...
export function runSetupWizard(): Config {
    ...
    if (!reactorPeripheral) {
        throw new Error("No reactor found, please connect a reactor to the computer and try again.");
    }

    if (turbinePeripherals.length === 0) {
        throw new Error("No turbines found, please connect at least one turbine to the computer and try again.");
    }

    if (monitorPeripherals.length !== turbinePeripherals.length + 1) {
        throw new Error("No monitors found, please connect a monitor for each turbine and the reactor and try again.");
    }

    const reactor = new Reactor(peripheral.getName(reactorPeripheral));
    const turbines = turbinePeripherals.map((turbinePeripheral) => new Turbine(peripheral.getName(turbinePeripheral)));
    const monitors = monitorPeripherals.map((monitorPeripheral) => new Monitor(peripheral.getName(monitorPeripheral)));

    renderPageOnAllMonitors(
        getTextPage("Setup Wizard", "Welcome to the setup wizard. This wizard will help you configure your reactor and turbines."),
        monitors
    );

    reactor.setAllControlRods(0);

    const targetFlowRate = turbines.reduce((acc, turbine) => acc + autotuneTurbine(monitors, turbine), 0);
    ...
    const config = {
        ...
        reactor: {
            ...
            monitor: getMonitorSelection("Please select the monitor to use for the reactor.", monitors),
            controlRodLevel: controlRodLevel,
        },
        turbines: turbines.map((turbine) => ({
            name: turbine.getName(),
            monitor: getMonitorSelection(`Please select the monitor to use for turbine ${turbine.getName()}.`, monitors),
...
function autotuneTurbine(monitors: Monitor[], turbine: Turbine, targetRPM = 1800): number {
    ...
    while (true) {
        ...
        if (Math.abs(actualRPM - targetRPM) < targetRPM * stableThresholdPercentage) {
...
function autotuneReactor(monitors: Monitor[], reactor: Reactor, targetFlowRate: number): number {
    const maxControlRodLevel = 100;
    const minStep = 0.01;
    const maxStep = 10;
    const stableThresholdPercentage = 0.0025;
    const requiredStableIterations = 10;
    ...
    while (true) {
        ...
        const actualFlowRate = reactor.getCoolantMaxTransitioned();

        if (Math.abs(actualFlowRate - targetFlowRate) < targetFlowRate * stableThresholdPercentage) {
            stableIterationCount++;
        ...
        if (actualFlowRate >= targetFlowRate) {
            reactor.setAllControlRods(Math.min(controlRodLevel + step, maxControlRodLevel));
        } else {
